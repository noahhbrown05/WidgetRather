...
| G2 | **Tech spike, in a simulator** (cloud Mac via MacinCloud ~$1/hr, or Appetize). The "answer saved to Supabase from the widget" half is **answered: it can't** (D-020, 2026-09-14) — the spike becomes: `expo-widgets` widget with 2 buttons → local pending vote → app flush → Supabase → widget updates, **plus do widget changes need a native rebuild?** (UNVERIFIED; it decides how Noah iterates on the widget, D-017) | Greg | Heavy |
